...
const Index: DocsItem[] = [
  {
    title: "What is Baitech Files?",
    sub: [
      "Baitech Files is a Web3.0 decentralized IPFS storage application.",
      "Baitech Files helps you to easily store your personal files on IPFS via Baitech decentralized storage network."
    ]
  },
  {
  ...
  },
  {
    ...
    sub: [
      "Baitech Files supports multi-wallets as Web3.0 identity to sign-in the application and use its Web3.0 Auth IPFS Gateway & Web3.0 Auth Baitech Pinner. ",
      asOption("Baitech Network – Browser Wallet"),
  ...
  },
  {
    ...
    sub: [
      ...
      asOption(
        "Smart Contract on various platforms, so that users are able to Deposit / Pay to use Baitech Files."
      ),
  ...
  {
    title: "About IPFS Web3-Auth Baitech Pinner",
    sub: [
      "IPFS W3Auth Pinning Service is a lightweight Web3-based authentication service based on IPFS remote pinning service and Baitech Network.",
  ...
  },
  {
    ...
    sub: [
      "Baitech Files is a decentralized Application, and it will NEVER store your Upload History and File Encryption Key on any remote server. Instead, they are cached on your local devices. This indicates that, if you sign-in Baitech Files with your wallet account on a new device, you will lose your Upload History and File Encryption Key from your previous device.",
...
function Docs(props: Props) {
  ...
  return (
    <SideLayout path={"/docs"}>
      <Segment basic className={className}>
        <User />
        <Accordion>
          {Index.map((item, index) => (
            <Item key={`docs_item_${index}`} item={item} index={index} />
          ))}
        </Accordion>
      </Segment>
    </SideLayout>
