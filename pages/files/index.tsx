...
function Index(p: { className?: string }) {
  ...
  const onClickUpFile = useCallback(() => _clickUploadFile(false), [
    _clickUploadFile
  ]);
  const onClickUpFolder = useCallback(() => _clickUploadFile(true), [
    _clickUploadFile
  ]);
  const _onInputFile = useCallback<FunInputFile>(
    e => {
  ...
  const _onInputImportFile = useCallback<FunInputFile>(
    e => {
      try {
  ...
  const _export = useCallback(() => {
    const blob = new Blob([JSON.stringify(wFiles.files)], {
      ...
      <Segment basic className={p.className}>
        ...
        <Segment basic className="contentPanel">
          <Segment
            basic
            textAlign={"center"}
            className={"font-sans-semibold uploadPanel"}
          >
            <input
            ...
            <div className={"upSlog"}>
              Upload and Store your File or Folder to IPFS via{" "}
              <a
                target="_blank"
          ...
          <Table basic={"very"}>
            ...
            <Table.Body className="font-sans-regular">
              {localFiles.pageList.map((f, index) => (
                <FileItem key={`files_item_${index}`} uc={uc} file={f} />
              ))}
            </Table.Body>
            ...
            <Table.Footer>
              <Table.Row>
                <Table.HeaderCell
                  colSpan={localFiles.totalPage > 1 ? 2 : 5}
                  className={"btns"}
                >
                  ...
                  <Popup
                    ...
                    trigger={
                      <span
                        className="cru-fo-help-circle"
                        style={{ fontSize: "1.3rem" }}
                      />
                    }
                    content={
                      "Baitech Files is a decentralized Application, and it will NEVER store your Upload History and File Encryption Key on any remote server. Instead, they are cached on your local devices. If you want to migrate your Upload History and File Encryption Key to a new device, use Export & Import function."
                    }
                  />
                </Table.HeaderCell>
                {localFiles.totalPage > 1 && (
                  <Table.HeaderCell colSpan="3" textAlign={"right"}>
                    <Pagination
                      totalPages={localFiles.totalPage}
                      activePage={localFiles.page}
                      ...
                      onPageChange={(_, { activePage }) =>
                        localFiles.setPage(activePage as number)
                      }
                    />
                  </Table.HeaderCell>
                )}
