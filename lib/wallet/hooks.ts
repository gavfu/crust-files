...
export function useFiles(key: KEYS_FILES = "files"): WrapFiles {
  ...
  return useMemo(() => ({ ...filesObj, setFiles, key }), [
    filesObj,
    setFiles,
    key
  ]);
...
export function useSign(wUser: WrapLoginUser): UseSign {
  ...
  useEffect(() => {
    ...
    if (wUser.wallet === "huochain") {
      setState(o => ({
        ...o,
        sign: async data => {
          return wUser.huochain.sign(data);
        }
      }));
    }
...
export function useLoginUser(key: KEYS = "files:login"): WrapLoginUser {
  ...
  const huochain = useMemo(() => new HuoChain(), []);
